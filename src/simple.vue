...
export default {
  ...
  methods: {
    handleFileChange(e) {
      ...
      fileIndex = 0; // 重置文件下标
      // 判断文件选择的个数
      if (this.limit && files.length > this.limit) { // 限制上传最大个数为20
      ...
      const postFiles = Array.prototype.slice.call(files); // 将有length属性的对象转换为数组
      console.log('handleFileChange -> postFiles', postFiles);
      postFiles.forEach((item) => {
        this.handleStart(item);
      });
    },
    handleStart(rawFile) { // 定义进度条状态
      // 给rawFile初始化部分自定义属性
    ...
    async handleUpload() { // 点击上传获得uploadFiles
      ...
      return new Promise(async (resolve, reject) => {
        const requestDataList = chunkData
          ...
          .map(({ fileHash, chunk, fileName, index }) => {
            const formData = new FormData();// 基于文件流 element-ui上传组件默认是基于文件流的
        ...
        try {
          const ret = await this.sendRequest(requestDataList, chunkData);// 向服务器发请求
        ...
        // 合并切片
        const isUpload = chunkData.some((item) => item.uploaded === false);// some查找到元素返回true，否则返回false
      ...
      return new Promise((resolve, reject) => {
        const handler = () => {
          ...
          if (forms.length) {
            ...
            console.log('sendRequest - > formData', formData);
            instance
    ...
    createFileChunk(file, size = chunkSize) {
      console.log('切片1', file);
      const fileChunkList = [];
      var count = 0;
      console.log('切片2', file.size);
      while (count < file.size) {
        fileChunkList.push({
          file: file.slice(count, count + size)// html5里面可以对file的size进行slice分割
        });
        count += size;
      }
    ...
    calculateHash(fileChunkList) {
      ...
      return new Promise((resolve) => {
        // this.worker = new Worker('./hash/md5.js');// 这个地方需要用到node里面的__dirname
        this.worker = new Worker('./hash.js'); // 调用生成hash值
